...
*Setup instructions for a gitian build of Bloom Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Bloom
...
to the Bloom GitHub release page.
...
- [Building Bloom Core](#building-bloomcoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/bloomcoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/bloomcoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/bloomcoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/bloomcoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/bloomcoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/bloomcoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/bloomcoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@bloomcoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/bloomcoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/bloomcoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/bloomcoin-osx-*-build.assert
```
