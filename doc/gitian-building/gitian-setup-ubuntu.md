...
The build script we'll be using is contained in the Bloom github repository ([contrib/gitian-build.py](https://github.com/bloomcoin-project/bloomcoin/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the Bloom repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/Bloom-Project/Bloom/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, bloomcoin-detached-sigs, and bloomcoin GitHub repos.
