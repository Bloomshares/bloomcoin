...
- [Development guidelines](#development-guidelines)
    - [General Bloom Core](#general-bloomcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Bloom Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Bloom Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/bloomcoin-project/bloomcoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_bloomcoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_bloomcoin --log_level=test_suite
$ valgrind -v --leak-check=full src/bloomcoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_bloomcoin.coverage/index.html`.
```
...
However, in open source software such as Bloom Core, where everyone uses
...
pay attention to for reviewers of Bloom Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Bloom Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Bloom Core so that they can be integrated
...
file descriptors it will cause problems with Bloom's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:Bloom-Project/Bloom.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `bloomcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `bloomcoin-cli` and the GUI debug console use this table to determine how to
