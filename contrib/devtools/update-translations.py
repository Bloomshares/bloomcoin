...
def remove_current_translations():
    ...
    This leaves bloomcoin_en.ts untouched.
    '''
...
def update_build_systems():
    ...
    '''
    filename_lang = [re.match(r'((bloomcoin_(.*)).ts)$', filename).groups() for (filename, filepath) in all_ts_files(include_source=True)]
    filename_lang.sort(key=lambda x: x[0])

    # update qrc locales
    with open('src/qt/bloomcoin_locale.qrc', 'w') as f:
