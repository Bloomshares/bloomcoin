...
#include "config/bloomcoin-config.h"
#endif

#include "qt/bloomcoin/bloomcoingui.h"
...
#include "qt/bloomcoin/splash.h"
#include "qt/bloomcoin/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("bloomcoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in bloomcoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in bloomcoin.qrc)
...
/** Class encapsulating Bloom Core startup and shutdown.
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        // Bloom: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. Bloom can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(bloomcoin_locale);
    Q_INIT_RESOURCE(bloomcoin);
    ...
    /// 6. Determine availability of data directory and parse bloomcoin.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("Bloom"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("Bloom"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // bloomcoin: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("Bloom didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
