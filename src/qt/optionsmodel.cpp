...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::Bloom);
    ...
    if (settings.contains("nAnonymizeIDChainCoinAmount") || reset)
        SoftSetArg("-anonymizebloomcoinamount", settings.value("nAnonymizeIDChainCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in bloomcoin.cpp during shotdown to also remove the window positions
