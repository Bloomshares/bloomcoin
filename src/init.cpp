...
void PrepareShutdown()
{
    ...
    util::ThreadRename("bloomcoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Bloom specific functionality (Masternodes, Budgeting) (0-1, default: %u)"), 0));
...
{
    util::ThreadRename("bloomcoin-loadblk");
...
/** Sanity checks
 *  Ensure that Bloom is running in a usable environment with all
...
#endif
    LogPrintf("Bloom version %s (%s)\n", version_string, CLIENT_DATE);
}

/** Initialize bloomcoin.
...
bool AppInit2()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. Bloom is shutting down."));

    std::string strDataDir = GetDataDir().string();

    // Make sure only a single Bloom process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. Bloom is probably already running."), strDataDir));
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                // Bloom: load previous sessions sporks if we have them.
