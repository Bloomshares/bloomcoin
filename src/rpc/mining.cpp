...
UniValue getgenerate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "It is set with the command line argument -gen (or bloomcoin.conf setting gen)\n"
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    //     throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Bloom is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Bloom is downloading blocks...");
